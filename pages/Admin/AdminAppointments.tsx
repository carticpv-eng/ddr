               ...
               {MOCK_APPOINTMENTS.map((apt) => (
                   <div key={apt.id} className="bg-[#121214] border border-gray-800 rounded-xl p-6 flex flex-col md:flex-row gap-6 hover:shadow-lg hover:border-gray-700 transition">
                       ...
                       <div className="flex flex-row md:flex-col gap-2 justify-center border-t md:border-t-0 md:border-l border-gray-800 pt-4 md:pt-0 md:pl-6 min-w-[140px]">
                           {apt.status === 'pending' && (
                               <>
                                <button className="flex-1 px-4 py-2 bg-green-600 hover:bg-green-500 text-white text-xs font-bold rounded uppercase transition">
                                    Confirmer
                                </button>
                                <button className="flex-1 px-4 py-2 bg-red-900/50 hover:bg-red-900 border border-red-800 text-red-200 text-xs font-bold rounded uppercase transition">
                                    Refuser
                                </button>
                               </>
                           )}
                           {apt.status !== 'pending' && (
                               <button className="flex-1 px-4 py-2 bg-gray-800 hover:bg-gray-700 text-gray-300 text-xs font-bold rounded uppercase transition">
                                   Archiver
                               </button>
                           )}
