    ...
    <div className="space-y-8 animate-fade-in-up pb-20">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6 border-b border-gray-800 pb-8">
        <div>
            ...
            <p className="text-gray-500 mt-2 text-sm">
                Suivi comptable des dons pour la <span className="text-brand-500 font-bold">Campagne École</span>.
            </p>
        </div>
        <div className="flex gap-3">
             <button className="px-5 py-2.5 bg-[#121214] border border-gray-700 text-gray-300 rounded-lg text-sm font-medium hover:text-white hover:border-gray-500 transition flex items-center gap-2">
                 <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                 Exporter CSV
             </button>
             <button className="px-5 py-2.5 bg-brand-600 text-white font-bold rounded-lg text-sm hover:bg-brand-500 transition shadow-[0_0_20px_rgba(234,88,12,0.3)] flex items-center gap-2">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="bg-[#121214] border border-gray-800 p-6 rounded-2xl relative overflow-hidden group hover:border-brand-500/30 transition">
              ...
              <span className="text-xs text-gray-500 uppercase tracking-widest font-bold">Volume Total (Affiché)</span>
              ...
              <div className="mt-4 flex items-center gap-2 text-xs text-green-500 bg-green-500/10 w-fit px-2 py-1 rounded border border-green-500/20">
                  <span>▲ +12%</span>
                  <span className="text-gray-500 border-l border-green-500/20 pl-2">vs mois dernier</span>
              </div>
          </div>
          ...
          <div className="bg-[#121214] border border-gray-800 p-6 rounded-2xl">
              ...
              <p className="text-xs text-gray-600 mt-4">Dons validés et encaissés.</p>
          </div>

          <div className="bg-[#121214] border border-gray-800 p-6 rounded-2xl">
              <span className="text-xs text-gray-500 uppercase tracking-widest font-bold">Panier Moyen</span>
              <p className="text-3xl font-mono font-bold text-gray-300 mt-2">{(totalAmount / (filteredDonations.length || 1)).toLocaleString()} F</p>
              <p className="text-xs text-gray-600 mt-4">Moyenne par donateur.</p>
          </div>
      </div>
      ...
      <div className="bg-[#121214] border border-gray-800 rounded-2xl overflow-hidden shadow-2xl">
                        ...
                        <tr key={don.id} className="hover:bg-gray-800/30 transition group">
                            <td className="px-6 py-4">
                                <span className="font-mono text-xs text-gray-500 bg-black px-2 py-1 rounded border border-gray-800 group-hover:border-gray-600 transition">
                                    #{don.transactionId || `TR-${don.id}849`}
                                </span>
                            </td>
                            <td className="px-6 py-4">
                            ...
                            <td className="px-6 py-4 text-right">
                                <button className="text-gray-500 hover:text-brand-500 p-2 rounded-lg hover:bg-gray-800 transition" title="Télécharger Reçu">
          ...
          {/* Pagination Footer */}
          <div className="bg-black border-t border-gray-800 p-4 flex justify-between items-center">
              <span className="text-xs text-gray-500">Affichage 1-{filteredDonations.length} sur {filteredDonations.length}</span>
              <div className="flex gap-2">
                  <button className="px-3 py-1 bg-gray-900 border border-gray-700 rounded text-xs text-gray-400 disabled:opacity-50" disabled>Précédent</button>
                  <button className="px-3 py-1 bg-gray-900 border border-gray-700 rounded text-xs text-gray-400 disabled:opacity-50" disabled>Suivant</button>
              </div>
          </div>
      </div>
    </div>
