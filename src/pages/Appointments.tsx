...
const Appointments = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
        ...
        const fullPhone = `${phoneCountry} ${formData.phone}`;
        ...
        const existing = JSON.parse(localStorage.getItem('ddr_appointments') || '[]');
        existing.push(newAppointment);
        localStorage.setItem('ddr_appointments', JSON.stringify(existing));
        
        // --- LOG ACTION ---
        logAction('NEW_RDV', `RDV demandé par ${formData.name} (${fullPhone}) pour le ${formData.date}`, 'success');
        // ------------------
        ...
        setErrorMsg("Une erreur est survenue lors de l'enregistrement local.");
        logAction('ERROR_RDV', `Echec demande RDV: ${error.message}`, 'danger');
        setLoading(false);
    }
  };
        ...
        <div className="min-h-screen bg-black flex items-center justify-center p-4">
            <div className="max-w-xl w-full bg-[#09090b] border border-brand-500/20 text-white p-12 rounded-3xl shadow-[0_0_60px_rgba(234,88,12,0.1)] text-center relative overflow-hidden">
                ...
                <p className="text-gray-400 text-lg mb-8 leading-relaxed">
                    Barak Allah Oufik <strong>{formData.name}</strong>. Votre demande a bien été transmise au secrétariat de la DDR (Local). Nous vous contacterons au <strong>{phoneCountry} {formData.phone}</strong> sous 24h pour confirmer le créneau.
                </p>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div className="group">
                                ...
                                <div className="flex gap-2">
                                    <select 
                                        className="bg-[#121214] border border-gray-800 rounded-xl px-3 text-white text-sm outline-none focus:border-brand-500"
                                        value={phoneCountry}
